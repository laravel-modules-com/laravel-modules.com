...
class ShowDocumentationController extends Controller
{
    public function __invoke(Request $request, string $version, ?string $page = null)
    {
        ...
        $navigation = Cache::remember("docs_nav_{$version}", now()->addDay(),
            fn () => $this->getNavigation($version)
        );
        ...
        try {
            $pageProperties = Cache::remember("docs_{$version}_{$page}", now()->addDay(),
                fn () => $this->getPageProperties($version, $page)
            );
    ...
    protected function getPageProperties($version, $page = null): array
    {
        ...
        //$content = $this->getMarkdownView("docs.{$markdownFileName}")->render();
        $content = file_get_contents(resource_path("views/docs/{$version}/{$page}.md"));
        ...
        $pageProperties['editUrl'] = "https://github.com/nWidart/laravel-modules/tree/main/resources/views/docs/{$version}/{$page}.md";
